...
async function getAllReceiptsDetls() {
  const { allReceiptsDetlsByBranch } = await graphQLClient.request(
    gql`
      query getReceiptsDetls($branch: String) {
        allReceiptsDetlsByBranch(branch: $branch) {
          rcpd_id
          rcpd_no
          rcpd_invno
          rcpd_invdate
  ...
  return allReceiptsDetlsByBranch;
}

async function getReceiptsDetlsByReptno(reptno) {
  const { allReceiptsDetlsByReptno } = await graphQLClient.request(
    gql`
      query getReceiptsDetls($reptno: String, $branch: String) {
        allReceiptsDetlsByReptno(reptno: $reptno, branch: $branch) {
          rcpd_id
          rcpd_no
          rcpd_invno
          rcpd_invdate
