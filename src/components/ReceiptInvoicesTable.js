...
const ReceiptInvoicesTable = ({
  ...
  const filteredData = batchdetlsstate.filter(
    item =>
      !item.ar_paid &&
      item.ar_custno &&
      item.ar_custno.toLowerCase().includes(filterText.toLowerCase())
  );
  ...
  const columns = [
    ...
    },
    {
      id: 8,
      name: 'Amount',
      selector: row => row.ar_total_amt,
      width: '120px',
      sortable: true,
      right: true,
      cell: row => <div>{currency(row.ar_total_amt).format()}</div>,
    },
    {
      id: 9,
      name: 'Balance Due',
      selector: row => row.ar_balance,
      width: '120px',
      sortable: true,
      right: true,
      cell: row => <div>{currency(row.ar_balance).format()}</div>,
    },
    {
      id: 10,
      name: 'Paid',
...
export default ReceiptInvoicesTable;
