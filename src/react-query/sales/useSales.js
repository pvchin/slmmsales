...
async function getSales() {
  const { allSalesByBranch } = await graphQLClient.request(
    gql`
      query getSales($branch: String) {
        allSalesByBranch(branch: $branch) {
          sls_id
          sls_no
          sls_so
          sls_date
  ...
  return allSalesByBranch;
}

async function getSalesByCustno(custno) {
  const { allSalesByCustno } = await graphQLClient.request(
    gql`
      query getSales($custno: String, $branch: String) {
        allSalesByCustno(custno: $custno, branch: $branch) {
          sls_id
          sls_no
          sls_so
          sls_date
