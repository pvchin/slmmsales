...
export function useAddSales(data) {
  const queryClient = useQueryClient();

  const { mutate } = useMutation({
    mutationFn: data => addSales(data),
    onSuccess: () => {
      Toast({
        title: 'New Sales being added!',
        status: 'success',
        customId: 'salesAdd',
      });
    },
    onError: () => {
      Toast({
        title: 'Sales Add Error! Please check your internet connection!',
        status: 'warning',
        customId: 'salesAddErr',
      });
    },
    onSettled: () => {
