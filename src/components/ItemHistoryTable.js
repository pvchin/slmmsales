...
const ItemHistoryTable = () => {
  ...
  const handleCalcTotals = item_no => {
    ...
    const qohqty = getValues('item_qoh_pc');
    const ucost = getValues('item_ucost_pc');
    totonhandamt = qohqty * ucost;
    itemshistory
  ...
  const update_ItemDetls = data => {
    ...
    setValue('item_qoh_pc', item_qoh_pc);
    setValue('item_ucost_pc', item_ucost_pc);
    setValue('item_uprice_pc', item_uprice_pc);
  ...
  return (
    ...
    >
      ...
      <Modal
        size="5x1"
        opened={isItemSearchOpen}
        onClose={onItemSearchClose}
        title="Items Search Table"
      >
        <ItemSearchTable
          state={state}
          setState={setState}
